...
(() => {
  ...
  function editor() {
    ...
    if(e.type==='device') {const x=e.key?device(e.key):null;body=field('name','Device name',x?.name)+select('kind','Type',[['server','Server'],['switch','Switch'],['other','Other']],x?.kind||'server');if(!x)body+=select('template','Template',[['blank','Empty / custom'],['vera','4 Node + 4 BF4 / 2 RJ45'],['switch','Switch ports']])+field('count','Switch port count',32,'number');}
    if(e.type==='import')body=`<p>\u52a0\u5165\u8a2d\u5099\u7684\u62d3\u64b2\u526f\u672c\uff0c\u4e0d\u8b8a\u66f4\u5eab\u5b58\u3002</p><div class="nt-checks">${machines.filter(m=>m.project===state.project&&!rack().devices.some(d=>d.inventory===m.name)).map(m=>`<label><input type="checkbox" name="inventory" value="${h(m.name)}">${h(m.name)}</label>`).join('')}</div>${select('template','Server template',[['blank','Empty / custom'],['vera','4 Node + 4 BF4 / 2 RJ45']])}${field('count','Switch port count',48,'number')}`;
  ...
  function makeDevice(name,kind,template,count,inventory='') {
    const d={id:id(),name,kind,inventory,nodes:[],ports:[]};
    if(template==='vera'&&kind==='server'){
      d.nodes=Array.from({length:4},(_,i)=>({id:id(),name:'Node '+(i+1),bf4:'BF4 #'+(i+1),host_os:'',host_bmc:'',dpu_os:'',dpu_bmc:''}));
      d.ports=['host','dpu'].map((role,i)=>({id:id(),name:'RJ45 #'+(i+1),role,nodes:d.nodes.map(n=>n.id)}));
    }
  ...
  async function submit() {
    ...
    if(e.type==='device') {const name=required('name'),kind=val('kind');if(e.key)Object.assign(device(e.key),{name,kind});else r.devices.push(makeDevice(name,kind,val('template'),kind==='switch'?number('count'):0));}
    if(e.type==='import') {const count=number('count'),names=checked('inventory');if(!names.length)throw Error('Select at least one device');names.forEach(name=>{const m=machines.find(m=>m.name===name),type=mgxTypeOf(m);r.devices.push(makeDevice(name,['server','switch'].includes(type)?type:'other',val('template'),count,name));});}
