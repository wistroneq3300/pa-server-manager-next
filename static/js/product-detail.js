...
(() => {
  ...
  RENDERERS.machine = function productMachine() {
    ...
    return `<div class="pd-workspace" data-system="${esc(name)}"><div class="pd-breadcrumb"><button onclick="machineBack()">System Manager</button><span>/</span><button onclick="machineBack()">${esc(b.project || '未分類')}</button><span>/</span><strong>${esc(name)}</strong></div><header class="pd-system-header"><div class="pd-system-title"><span class="pd-eyebrow">${level} ${rack ? 'RACK LEVEL' : 'SYSTEM LEVEL'} / SYSTEM WORKSPACE</span><h1>${esc(name)}<span class="pd-level-pill">${level}</span></h1><p>${esc(identity.label || os.distro || 'System platform')} <span>·</span> ${esc(hw.cpu?.model || b.mgx_type || 'Managed system')}</p></div><div class="pd-header-status">${stateDot(b.os_alive,b.os_alive ? 'System online' : 'System offline')}<div><span>PROJECT</span><b>${esc(b.project || '未分類')}</b></div></div></header><div class="pd-workspace-grid"><div class="pd-main"><nav class="pd-tabs" role="tablist" aria-label="系統工作區">${tabs.map(([key,label,zh]) => `<button id="pd-tab-${key}" role="tab" aria-selected="${selected===key}" aria-controls="pd-panel-${key}" tabindex="${selected===key?'0':'-1'}" class="${selected===key?'is-active':''}" data-pd-tab="${key}" onclick="productDetailTab('${key}')"><span>${label}</span><small>${zh}</small></button>`).join('')}</nav>${tabs.map(([key]) => `<section id="pd-panel-${key}" role="tabpanel" aria-labelledby="pd-tab-${key}" class="pd-tab-panel${selected===key?' is-active':''}"${selected!==key?' hidden':''}>${panels[key]}</section>`).join('')}</div><aside class="pd-operations p-surface"><div class="pd-ops-heading"><span class="pd-eyebrow">COMMAND DECK</span><h2>系統操作</h2><span class="pd-command-line"></span></div><div class="pd-operation-group">${operationButton(findAction('openTermDialog'),'開啟 Terminal','terminal',true)}${b.bmc_ip ? `<button class="pd-operation" onclick="window.openKvmSolo && window.openKvmSolo(${quote(name)})">${glyph('monitor','▣')}<span>KVM</span><b>↗</b></button>`:''}${operationButton(findAction('openAssignTask'),'指派測試任務','clipboard')}${!m.passive ? `<button class="pd-operation" onclick="productDetailTab('overview'); runDiagnose(${quote(name)})">${glyph('activity','⌁')}<span>系統診斷</span><b>↗</b></button>`:''}</div>${powerActions ? `<div class="pd-power-group"><span class="pd-eyebrow">POWER MANAGEMENT</span>${powerActions.outerHTML}</div>`:''}<div class="pd-ops-context">${kv('PROJECT LEVEL',rack?'L11 / Rack Level':'L10 / System Level')}${kv('COMPONENT',b.mgx_type || 'server')}${rack && b.rack_u ? kv('RACK POSITION',`U${b.rack_u} · ${b.rack_size || 1}U`) : kv('UPTIME',os.uptime)}</div><div class="pd-refresh-action">${operationButton(findAction('machineRefresh'),'重新整理資料','refresh')}</div><div class="pd-ops-foot"><i></i>ENGINEERING WORKSPACE</div></aside></div></div>`;
  };
